...
def load_tf_weights_in_bert(model, config, tf_checkpoint_path):           #传入数据
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, pos_tag_embedding = False, senti_embedding = False, polarity_embedding = False):
        ...
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size, padding_idx=0)   #创造词嵌入层  词汇表大小和隐藏层大小
        self.position_embeddings = nn.Embedding(config.max_position_embeddings, config.hidden_size)   #位置信息嵌入层
        self.token_type_embeddings = nn.Embedding(config.type_vocab_size, config.hidden_size)             #标记类型的嵌入
