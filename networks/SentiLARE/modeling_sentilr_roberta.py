...
from networks.SentiLARE.modeling_sentilr import BertEmbeddings, BertLayerNorm, BertModel, BertPreTrainedModel, gelu
from transformers import RobertaConfig
from transformers import ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST
...
from tensorflow.keras.layers import Conv1D, Dense, LayerNormalization, ReLU


class MultiLevelEncoder(tf.keras.layers.Layer):
    def __init__(self, filters, kernel_size, num_heads, dropout_rate=0.1):
        super(MultiLevelEncoder, self).__init__()
        self.conv1 = Conv1D(filters=filters, kernel_size=kernel_size, padding='same')
        self.multi_head_attention = tf.keras.layers.MultiHeadAttention(num_heads=num_heads, key_dim=filters)
        self.relu = ReLU()
        self.conv2 = Conv1D(filters=filters, kernel_size=kernel_size, padding='same')
        self.conv3 = Conv1D(filters=filters, kernel_size=kernel_size, padding='same')
...
class RobertaForSequenceClassification(BertPreTrainedModel):
    ...
    config_class = RobertaConfig
    pretrained_model_archive_map = ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST #加载预训练模型
    base_model_prefix = "roberta"
